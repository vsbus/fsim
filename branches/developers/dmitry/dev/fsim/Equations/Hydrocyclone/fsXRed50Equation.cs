...
namespace Equations.Hydrocyclone
{
    public class fsXRed50Equation : fsCalculatorEquation
    {
         ...
         *             \                                           \ pi * etaF * n /                                                    /
         * 
         * with respect to the parameter xRed50 given as input parameters 
         ...
         *        \                                \ pi * etaF * n /             /
         * 
         *   B = 2 * (cu / c - 1);  E = (2 * (ln(sigmaG)^2 + ln(sigmaS)^2))^(1/2);  C = ln(xg) / E .          
         *    
         ...
         *                                         \   \     1 + erf(C - ln(xRed50) / E) / /
         * 
         * Put x = C - ln(xRed50) / E. Then (eq.3) can be written in the form
         * 
         *                   /   /         a3     \ \
         *   x = a1 - a2 * ln| ln| 1 + ---------- | |        (eq.4)
         *                   \   \     1 + erf(x) / /
         * 
         * where a1 = C - ln(A) / E = ln(xg / A) / E;   a2 = alpha2 / 2 / E >= 0;  a3 = B >= 0 (since cu >= c) 
         */
        ...
        class xCalculationFunction : fsFunction
        {
             ...
             * To calculate appropriately the above function for negative x < -7
            ...
            { 
                ...
                {
                    x = -x;
                    return fsValue.Log(x * x + fsValue.Log(a * Math.Sqrt(Math.PI) * 
                                                   (x + 0.5 / 
                                                        (x + x / (x * x + 1.5 / 
                                                                          (1 + 2 / (x * x + 2.5 / 
                                                                                           (1 + 3 / (x * x + 4)
        ...
        private void xRed50Formula()
        {
            ...
            fsValue E = fsValue.Sqrt(2 * (fsValue.Sqr(fsValue.Log(m_sigmaG.Value)) + fsValue.Sqr(fsValue.Log(m_sigmaS.Value))));
            fsValue C = fsValue.Log(m_xG.Value) / E;

            fsValue a1 = C - fsValue.Log(A) / E;
            fsValue a2 = m_alpha2.Value / 2 / E;
            fsValue a3 = B;

            fsValue lowerBound = new fsValue(-500);
